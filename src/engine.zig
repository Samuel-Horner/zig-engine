...
pub fn init(allocator: std.mem.Allocator, window_width: c_int, window_height: c_int, name: [*:0]const u8) !void {
    ...
    // Init GLFW
    try glfw.init();
    std.log.debug("Initialised GLFW {s}", .{glfw.getVersionString()});
...
//
//         std.log.debug("Created compute program {}.", .{compute_program.id});
//
...
//
//         if (location == -1) {
//             std.log.err("Failed to find uniform \"{s}\" in program {}.", .{ name, self.id });
...
//     std.log.debug("Registered new frame buffer size callback.", .{});
// }
//
// pub var window: Window = undefined;
//
...
//     std.log.debug("Window resized to {}x{}.", .{ width, height });
//
